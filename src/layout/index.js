...
import { ADMIN_NAV_LIST, APP_ROLES, THEME_MODE, URL, USER_NAV_LIST } from '../constants';
    ...
    <div className={`flex h-screen bg-white`}>
      <div
        className={`py-10 pt-20 border-r md:w-72 md:block md:fixed md:inset-y-0 md:left-0 transition-all duration-200 transform translate-x-0 overflow-y-auto ${
          sidebarOpen ? 'translate-x-0 ease-out transition-medium' : '-translate-x-full ease-in'
        } ${
        ...
        <nav className="mt-4">
          <ul className="px-5 pt-3">
            {layout === APP_ROLES.USER
              ? USER_NAV_LIST(layout, route).map((menu) => <MenuItem key={menu.id} menu={menu} />)
              : ''}
            {layout === APP_ROLES.ADMIN
              ? ADMIN_NAV_LIST(layout, route).map((menu) => <MenuItem key={menu.id} menu={menu} />)
              : ''}
          </ul>
        </nav>
      </div>
      ...
      <div className="relative min-h-screen md:flex md:flex-col md:flex-auto md:w-0 flex-grow">
        ...
        <main className={`p-4 flex-grow bg-white h-screen overflow-y-auto text-gray-600`}>
          {children}
        </main>
      </div>
    </div>
